...
int sc_main(int argc, char** argv){
	...
	median my_median("my_median"); //median filter module
	...
	sc_signal<bool> go_median; // go signal for median filter
	sc_signal<bool> done_median; // done signal for median filter
	...
	sc_signal<sc_dt::sc_int<24> > pixel_out_median; 
	...
	my_median.go(go_median);
	my_median.done(done_median);
	my_median.clk(clk);
	my_median.pixel_in(dataout_ram0);
	my_median.pixel_out(pixel_out_median);

	ram1.en(en_ram1);
	ram1.rw(rw_ram1);
	ram1.clk(clk);
	ram1.addr(addr_ram1);
	ram1.data_in(pixel_out_median);
	...
	num_iteration = num_pixel/KERNEL_SIZE;
